...
// ----------------------------------------------------------------------------
__device__
void simulation_init(int i, int j, int k, Substates &Q, Parameters &P)
{
...
// ----------------------------------------------------------------------------
  ...
  #define GET3D_Q_h(xi, xj, xk) (GET3D_SHARED(Q.h, s__Q_h, ROWS, COLS, i+xi, j+xj, k+xk, s__rows, s__cols, s__slices, s__i+xi, s__j+xj, s__k+xk ))
  #define GET3D_Q_k(xi, xj, xk) (GET3D_SHARED(Q.k, s__Q_k, ROWS, COLS, i+xi, j+xj, k+xk, s__rows, s__cols, s__slices, s__i+xi, s__j+xj, s__k+xk ))
  #define BUF_GET3D_Q_F(n, xi, xj, xk) (BUF_GET3D_SHARED(Q.F, s__Q_F, ROWS, COLS, SLICES, n, i+xi, j+xj, k+xk, s__rows, s__cols, s__slices, s__i+xi, s__j+xj, s__k+xk ))
#else
...
void compute_flows(int i, int j, int k, int s__i, int s__j, int s__k, Substates &Q,
                   int s__rows, int s__cols, Parameters &P)
#elif defined(CUDA_VERSION_TILED_NO_HALO)
...
#else
void compute_flows(int i, int j, int k, Substates &Q, Parameters &P)
#endif
{
  int k_inv = (SLICES-1) - k;
  double Delta_h = 0.0;
  ...
  if (k_inv > P.ZFONDO && h > GET3D_Q_h(Xi_6, Xj_6, Xk_6))
  {
    Delta_h = h - GET3D_Q_h(Xi_6, Xj_6, Xk_6);
    BUF_SET3D(Q.F, ROWS, COLS, SLICES, 0, i, j, k, Delta_h);
  }

  if (k_inv < P.ZSUP && h > GET3D_Q_h(Xi_5, Xj_5, Xk_5))
  {
    Delta_h = h - GET3D_Q_h(Xi_5, Xj_5, Xk_5);
    BUF_SET3D(Q.F, ROWS, COLS, SLICES, 1, i, j, k, Delta_h);
  }

  if (i > P.XW && h > GET3D_Q_h(Xi_1, Xj_1, Xk_1))
  {
    Delta_h = h - GET3D_Q_h(Xi_1, Xj_1, Xk_1);
    BUF_SET3D(Q.F, ROWS, COLS, SLICES, 2, i, j, k, Delta_h);
  }

  if (i < P.XE && h > GET3D_Q_h(Xi_4, Xj_4, Xk_4))
  {
    Delta_h = h - GET3D_Q_h(Xi_4, Xj_4, Xk_4);
    BUF_SET3D(Q.F, ROWS, COLS, SLICES, 3, i, j, k, Delta_h);
  }

  if (j > P.YIN && h > GET3D_Q_h(Xi_2, Xj_2, Xk_2))
  {
    Delta_h = h - GET3D_Q_h(Xi_2, Xj_2, Xk_2);
    BUF_SET3D(Q.F, ROWS, COLS, SLICES, 4, i, j, k, Delta_h);
  }

  if (j < P.YOUT && h > GET3D_Q_h(Xi_3, Xj_3, Xk_3))
  {
    Delta_h = h - GET3D_Q_h(Xi_3, Xj_3, Xk_3);
    BUF_SET3D(Q.F, ROWS, COLS, SLICES, 5, i, j, k, Delta_h);
  }
}
// ----------------------------------------------------------------------------
...
#endif
{
  ...
  ks = GET3D(Q.ks, ROWS, COLS, i, j, k);
  h_next = GET3D(Q.h, ROWS, COLS, i, j, k);

  double currentK = GET3D_Q_k(0, 0, 0);
  double currentDQDH = GET3D(Q.dqdh, ROWS, COLS, i, j, k);

  temp_value = ((currentK + GET3D_Q_k(Xi_4, Xj_4, Xk_4)) / 2.0) * currentDQDH;
  h_next = h_next - (BUF_GET3D_Q_F(3, 0, 0, 0) / (P.lato * P.lato)) * P.delta_t * temp_value;
  h_next = h_next + (BUF_GET3D_Q_F(2, Xi_4, Xj_4, Xk_4) / (P.lato * P.lato)) * P.delta_t * temp_value;

  temp_value = ((currentK + GET3D_Q_k(Xi_1, Xj_1, Xk_1)) / 2.0) * currentDQDH;
  h_next = h_next - (BUF_GET3D_Q_F(2, 0, 0, 0) / (P.lato * P.lato)) * P.delta_t * temp_value;
  h_next = h_next + (BUF_GET3D_Q_F(3, Xi_1, Xj_1, Xk_1) / (P.lato * P.lato)) * P.delta_t * temp_value;

  if( k_inv != P.ZSUP )
  {
    temp_value = ((currentK +GET3D_Q_k(Xi_5, Xj_5, Xk_5)) / 2.0) * currentDQDH;
    h_next = h_next - (BUF_GET3D_Q_F(1, 0, 0, 0) / (P.lato * P.lato)) * P.delta_t * temp_value;
    h_next = h_next + (BUF_GET3D_Q_F(0, Xi_5, Xj_5, Xk_5) / (P.lato * P.lato)) * P.delta_t * temp_value;
  }

  if( k_inv != P.ZFONDO )
  {
    temp_value = ((currentK + GET3D_Q_k(Xi_6, Xj_6, Xk_6)) / 2.0) * currentDQDH;
    h_next = h_next - (BUF_GET3D_Q_F(0, 0, 0, 0) / (P.lato * P.lato)) * P.delta_t * temp_value;
    h_next = h_next + (BUF_GET3D_Q_F(1, Xi_6, Xj_6, Xk_6) / (P.lato * P.lato)) * P.delta_t * temp_value;
  }

  temp_value = ((currentK + GET3D_Q_k(Xi_3, Xj_3, Xk_3)) / 2.0) * currentDQDH;
  h_next = h_next - (BUF_GET3D_Q_F(5, 0, 0, 0) / (P.lato * P.lato)) * P.delta_t * temp_value;
  h_next = h_next + (BUF_GET3D_Q_F(4, Xi_3, Xj_3, Xk_3) / (P.lato * P.lato)) * P.delta_t * temp_value;

  temp_value = ((currentK + GET3D_Q_k(Xi_2, Xj_2, Xk_2)) / 2.0) * currentDQDH;
  h_next = h_next - (BUF_GET3D_Q_F(4, 0, 0, 0) / (P.lato * P.lato)) * P.delta_t * temp_value;
  h_next = h_next + (BUF_GET3D_Q_F(5, Xi_2, Xj_2, Xk_2) / (P.lato * P.lato)) * P.delta_t * temp_value;

  if (k_inv == P.ZSUP && i <= ROWS * 0.7 && i > ROWS * 0.3 && j <= COLS * 0.7 && j > COLS * 0.3)
  {
